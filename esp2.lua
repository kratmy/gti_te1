		...
		if Char and Hum and Hum.Health > 0 then
			...
			if Root then
				...
				if OnS then
					...
					-- BOX (Только если включены Draw Boxes)
					...
					-- NAME & DIST (Только если включены Show Names / Show Distance)
					-- Используем Toggles.SelfNameAndDist.Value как на твоем скрине
					...
					if Toggles.ShowDist and Toggles.ShowDist.Value then
						if not IsSelf or (IsSelf and Toggles.SelfNameAndDist and Toggles.SelfNameAndDist.Value) then
							...
							local YOff = (Options.HealthBarSide and Options.HealthBarSide.Value == 'Bottom') and 12 or 2
					...
					if Toggles.ShowHPText and Toggles.ShowHPText.Value then
						if not IsSelf then -- Обычно на себе текст ХП не рисуют, но можно добавить проверку
					...
					-- TRACERS (Только если включены Tracers в Extra)
					...
					-- CHAMS (Только если включены Chams в Extra)
